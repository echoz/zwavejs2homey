...
exports.extractHaDiscoverySubsetFromSource = extractHaDiscoverySubsetFromSource;
exports.extractHaDiscoverySubsetFromFile = extractHaDiscoverySubsetFromFile;
const node_fs_1 = __importDefault(require("node:fs"));
const node_path_1 = __importDefault(require("node:path"));
const platform_output_policy_1 = require("./platform-output-policy");
const COMMAND_CLASS_NUMBERS = {
    BASIC: 32,
    BATTERY: 128,
...
const WINDOW_COVERING_POSITION_PROPERTY_KEYS = [
    'inboundBottom',
...
function parseStringSet(content) {
    return [...content.matchAll(/"([^"]+)"|'([^']+)'/g)]
        .map((match) => (match[1] ?? match[2] ?? '').trim())
        .filter((value) => value.length > 0);
}
function parsePropertyToken(content) {
    const trimmed = content.trim();
    if (trimmed === 'None')
        return null;
    if (PROPERTY_TOKEN_MAP[trimmed] !== undefined)
        return PROPERTY_TOKEN_MAP[trimmed];
    ...
    if (!Number.isNaN(dec) && String(dec) === trimmed)
        return dec;
    return undefined;
}
function parseCommandClassSet(content) {
...
function parsePropertySet(content) {
...
function parsePropertyKeySet(content) {
    const parsed = [];
    for (const token of content.split(',')) {
...
function parseInlineValueSchema(fragment) {
    ...
    if (typeMatch && VALUE_TYPE_MAP[typeMatch[1]])
        metadata.type = VALUE_TYPE_MAP[typeMatch[1]];
    if (readableMatch)
        metadata.readable = readableMatch[1] === 'True';
    if (writeableMatch)
        metadata.writeable = writeableMatch[1] === 'True';
    if (Object.keys(metadata).length > 0)
        parsed.metadata = metadata;
    return parsed;
}
function parseEntrySemantics(block) {
    ...
    const entityRegistryEnabledDefaultMatch = block.match(/\bentity_registry_enabled_default=(True|False)\b/);
    const semantics = {};
    if (allowMultiMatch)
        semantics.allowMulti = allowMultiMatch[1] === 'True';
    if (assumedStateMatch)
        semantics.assumedState = assumedStateMatch[1] === 'True';
...
function parseValueSchemaRef(block, fieldName) {
    if (fieldName === 'primary_value') {
        ...
        if (aliasMatch) {
            ...
            if (!alias) {
                return {
                    ...
                    unsupportedReason: 'unsupported-primary-value-alias',
                };
            }
            const value = {
        ...
        if (!inlineMatch) {
            return {
                ...
                unsupportedReason: 'unsupported-primary-value-shape',
            };
        }
        ...
        if (!value) {
            return {
                ...
                unsupportedReason: 'unsupported-primary-value-inline',
            };
        }
        return { value: value };
    }
    ...
    for (const match of inlineItems) {
        ...
        if (!value) {
            return {
                ...
                unsupportedReason: 'unsupported-companion-inline',
            };
        }
        parsed.push({
    ...
    if (inlineItems.length === 0 && listBody.trim().length > 0) {
        return {
            ...
            unsupportedReason: 'unsupported-companion-shape',
        };
    }
    return { values: parsed };
}
function parseDeviceMatch(block) {
    ...
    const fwMatch = block.match(/firmware_version=(?:FirmwareVersionRange\()?min_version=\"([^\"]+)\"[\s\S]*?max_version=\"([^\"]+)\"/);
    const result = {};
    if (manufacturerMatch)
        result.manufacturerId = parseHexSet(manufacturerMatch[1])[0];
    if (productIdMatch)
        result.productId = parseHexSet(productIdMatch[1])[0];
    if (productTypeMatch)
        result.productType = parseHexSet(productTypeMatch[1])[0];
    if (deviceClassGenericMatch) {
        const parsed = parseStringSet(deviceClassGenericMatch[1]);
        if (parsed.length > 0)
            result.deviceClassGeneric = parsed;
    }
    if (deviceClassSpecificMatch) {
        const parsed = parseStringSet(deviceClassSpecificMatch[1]);
        if (parsed.length > 0)
            result.deviceClassSpecific = parsed;
    }
    if (fwMatch)
        result.firmwareVersionRange = { min: fwMatch[1], max: fwMatch[2] };
...
function buildEntryFromBlock(source, block, sourceRefBase) {
    ...
    try {
        ...
        const output = (0, platform_output_policy_1.resolveHaPlatformOutput)(platform, {
        ...
        try {
            ...
            return {
                unsupported: {
                    sourceRef,
                    reason: error instanceof Error && error.name === 'UnsupportedCompanionAliasError'
...
function extractHaDiscoverySubsetFromSource(sourceText, sourceRefBase) {
...
function extractHaDiscoverySubsetFromFile(filePath) {
    const sourceText = node_fs_1.default.readFileSync(filePath, 'utf8');
    const sourceRefBase = node_path_1.default.relative(process.cwd(), filePath) || filePath;
