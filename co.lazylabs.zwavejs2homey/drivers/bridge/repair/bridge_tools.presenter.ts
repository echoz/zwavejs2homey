interface BridgeToolsPresenterRoot {
  Zwjs2HomeyUi?: {
    bridgeToolsPresenter?: BridgeToolsPresenter;
  };
}

type FilterMode = 'action-needed' | 'all';
...
interface NodeViewRow {
  ...
  recommendationTone: 'ok' | 'warn' | 'danger';
  recommendationReason: string;
  mappingSummary: string;
  mappingDetail: string;
  mappingSkippedSummary: string;
}

interface BridgeToolsViewModel {
  loading: boolean;
  status: string;
  tone: StatusTone;
  error: string | null;
  subtitle: string;
  runtimeRows: KvRow[];
  runtimeAdvancedRows: KvRow[];
  summaryRows: KvRow[];
  recommendationCodeRows: KvRow[];
  mappingSkipReasonRows: KvRow[];
  filterActionLabel: string;
  filterAllLabel: string;
  nodesMeta: string;
  nodes: NodeViewRow[];
  nodesEmptyMessage: string | null;
}

interface BridgeToolsState {
  loading: boolean;
  snapshot: any | null;
  error: string | null;
  status: string;
...
type BridgeToolsEvent =
  | { type: 'load_start' }
  | { type: 'load_success'; snapshot: any }
  | { type: 'load_error'; message: string }
  | { type: 'set_filter'; filterMode: FilterMode }
  | { type: string; [key: string]: unknown };

interface BridgeToolsPresenter {
  createInitialState: () => BridgeToolsState;
  reduce: (state: BridgeToolsState, event: BridgeToolsEvent) => BridgeToolsState;
  buildViewModel: (state: BridgeToolsState) => BridgeToolsViewModel;
}
...
function asTimeOrNull(value: unknown): string | null {
...
function recommendationPriority(node: any): number {
  if (node?.recommendation?.backfillNeeded) return 0;
  if (node?.recommendation?.available) return 1;
  return 2;
}

function recommendationLabel(node: any): string {
  if (node?.recommendation?.backfillNeeded) return 'Backfill Marker';
  if (node?.recommendation?.available) return 'Adopt Update';
  return 'No Action';
}

function recommendationTone(node: any): 'ok' | 'warn' | 'danger' {
  if (node?.recommendation?.backfillNeeded) return 'warn';
  if (node?.recommendation?.available) return 'danger';
  return 'ok';
}

function recommendationReasonLabel(node: any): string {
  const reasonLabel =
    node?.recommendation && typeof node.recommendation.reasonLabel === 'string'
      ? node.recommendation.reasonLabel.trim()
      : '';
  if (reasonLabel.length > 0) return reasonLabel;
...
function nodeIdentityLabel(node: any): string {
  const manufacturer =
    node?.node && typeof node.node.manufacturer === 'string' ? node.node.manufacturer.trim() : '';
  const product =
    node?.node && typeof node.node.product === 'string' ? node.node.product.trim() : '';
  if (manufacturer.length > 0 && product.length > 0) return `${manufacturer} ${product}`;
  if (manufacturer.length > 0) return manufacturer;
  if (product.length > 0) return product;
  return 'Unknown device';
}

function nodeStateLabel(node: any): string {
  const parts: string[] = [];
  if (node?.node?.ready === true) parts.push('Ready');
  else if (node?.node?.ready === false) parts.push('Not ready');
  if (node?.node?.isFailed === true) parts.push('Failed');
  else if (node?.node?.isFailed === false) parts.push('Healthy');
  if (
    node?.node &&
...
function profileMatchLabel(node: any): string {
  const matchBy =
    node?.profile && typeof node.profile.matchBy === 'string' ? node.profile.matchBy : null;
  const matchKey =
    node?.profile && typeof node.profile.matchKey === 'string' ? node.profile.matchKey : null;
  if (!matchBy && !matchKey) return 'n/a';
  return `${asText(matchBy)} / ${asText(matchKey)}`;
}

function skipReasonSummary(skipReasons: unknown): string {
...
function profileConfidenceLabel(node: any): string {
  const attribution =
  ...
  const confidence =
    node?.profile && typeof node.profile.confidence === 'string'
      ...
      : '';
  if (confidence === 'curated') return 'Project rule match';
  if (confidence === 'ha-derived') return 'Home Assistant-derived rule match';
  if (confidence === 'generic') return 'Generic fallback rule';
  return 'Unknown rule match level';
}

function profileSourceLabel(node: any): string {
  const attribution =
    node?.profileAttribution && typeof node.profileAttribution === 'object'
      ? node.profileAttribution
      : null;
  if (attribution && attribution.sourceLabel) return String(attribution.sourceLabel);
...
function runtimeStatusLabel(snapshot: any): string {
...
function runtimeInferencePolicyLabel(): string {
  return 'Compiled profiles only + safe fallback (no runtime generic inference)';
}

function diagnosticsRefreshStatusLabel(snapshot: any): string {
  const refresh =
  ...
  if (refresh && typeof refresh === 'object') {
    if (
      typeof refresh.lastFailureReason === 'string' &&
...
function buildCurationStatusSummary(curation: any): string {
  ...
  if (hasEntry) return 'No Change';
  return 'No Override';
}

function buildBridgeSnapshotWarnings(snapshot: any): string[] {
  ...
  const diagnosticsRefresh =
    snapshot.runtime && typeof snapshot.runtime === 'object'
      ? (snapshot.runtime as { diagnosticsRefresh?: unknown }).diagnosticsRefresh
      : null;
  if (
    diagnosticsRefresh &&
    typeof diagnosticsRefresh === 'object' &&
    typeof (diagnosticsRefresh as { lastFailureReason?: unknown }).lastFailureReason === 'string' &&
    (diagnosticsRefresh as { lastFailureReason: string }).lastFailureReason.trim().length > 0
  ) {
    warnings.push(
      `Diagnostics refresh last failed: ${(diagnosticsRefresh as { lastFailureReason: string }).lastFailureReason.trim()}`,
    );
  }
...
function toNodeViewRow(node: any): NodeViewRow {
  ...
  const location =
    node?.node && typeof node.node.location === 'string' && node.node.location.trim()
      ? node.node.location.trim()
      : null;
  const mappingSummary = `${asText(node?.mapping?.inboundEnabled)}/${asText(
    node?.mapping?.inboundConfigured,
  )} in, ${asText(node?.mapping?.outboundEnabled)}/${asText(node?.mapping?.outboundConfigured)} out`;
  return {
    nodeLabel,
    ...
    mappingSummary,
    mappingDetail: `Capabilities: ${asText(node?.mapping?.capabilityCount)} · Vertical Slice: ${
      node?.mapping?.verticalSliceApplied ? 'Yes' : 'No'
    }`,
...
(function attachBridgeToolsPresenter(
  root: BridgeToolsPresenterRoot | undefined,
  factory: () => BridgeToolsPresenter,
) {
  ...
  typeof window !== 'undefined'
    ? (window as unknown as BridgeToolsPresenterRoot)
    : typeof global !== 'undefined'
      ? (global as unknown as BridgeToolsPresenterRoot)
      : ({} as BridgeToolsPresenterRoot),
  function createBridgeToolsPresenter(): BridgeToolsPresenter {
    function createInitialState(): BridgeToolsState {
      return {
        loading: false,
        snapshot: null,
        error: null,
    ...
    function reduce(state: BridgeToolsState, event: BridgeToolsEvent): BridgeToolsState {
      ...
      if (event.type === 'set_filter') {
        const filterMode: FilterMode = event.filterMode === 'all' ? 'all' : 'action-needed';
        return {
          ...state,
          filterMode,
        };
    ...
    function buildViewModel(state: BridgeToolsState): BridgeToolsViewModel {
      if (!state.snapshot) {
        return {
      ...
      const recommendationCodeRows = toKvRows(
        nodes
          .filter(
            (node) => !isMissingValue(node?.recommendation && (node.recommendation as any).reason),
          )
          .map((node) => {
            const nodeLabel = node?.nodeId !== null ? `Node ${node.nodeId}` : 'Node n/a';
            return [nodeLabel, node?.recommendation?.reason, 'mono'] as [string, unknown, string];
          }),
        { omitEmpty: true },
      );

      const mappingSkipReasonRows = toKvRows(
        Object.entries(snapshot?.nodeSummary?.skipReasons || {})
          .filter(([, count]) => typeof count === 'number' && count > 0)
          .sort((left, right) => (right[1] as number) - (left[1] as number))
          .map(([reason, count]) => [reason, count, 'mono'] as [string, unknown, string]),
        { omitEmpty: true },
      );
