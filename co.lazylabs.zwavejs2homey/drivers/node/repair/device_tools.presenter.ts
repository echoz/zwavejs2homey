...
interface DeviceToolsViewModel {
  ...
  actionHint: string;
  lockExtensionRows: KvRow[];
  lockSetCodeDisabled: boolean;
  lockRemoveCodeDisabled: boolean;
  lockSetStateDisabled: boolean;
...
const actionReasonLabels: Record<string, string> = {
  ...
  'slot-state-write-value-unsupported':
    'This lock does not support the requested slot state value.',
...
const extensionReadReasonLabels: Record<string, string> = {
...
function extensionReasonLabel(reason: unknown): string {
...
function getExtensionReadSection(read: any, sectionId: string): any | null {
  if (!read || typeof read !== 'object') return null;
  if (!Array.isArray(read.sections)) return null;
  const section = read.sections.find(
    (entry: any) => entry && typeof entry === 'object' && entry.sectionId === sectionId,
  );
  return section || null;
}

function lockExtensionActionAvailable(lockRead: any): boolean {
  if (!lockRead || typeof lockRead !== 'object') return false;
...
function buildLockExtensionRows(lockRead: any): KvRow[] {
  ...
  const summary =
    userCodeSection && typeof userCodeSection.summary === 'object' ? userCodeSection.summary : {};
  const diagnostics =
    lockoutSection && typeof lockoutSection.diagnostics === 'object'
      ? lockoutSection.diagnostics
      : {};
  const warnings = Array.isArray(diagnostics.warnings)
    ? diagnostics.warnings.map((entry: unknown) => String(entry))
    : [];
...
function buildLockActionHint(lockRead: any): string {
  if (!lockRead || typeof lockRead !== 'object') {
