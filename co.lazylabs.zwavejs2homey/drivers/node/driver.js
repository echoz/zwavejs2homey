...
module.exports = (_a = class NodeDriver extends homey_1.default.Driver {
        ...
        resolvePairRuntimeBridges(app) {
            const runtimeClientsByBridgeId = new Map();
            const setRuntime = (bridgeId, client) => {
            ...
            const preferredBridgeId = app.getBridgeId?.() ?? pairing_1.ZWJS_DEFAULT_BRIDGE_ID;
        ...
        async onPairListDevices() {
            ...
            if (connectedRuntimes.length === 0) {
                this.error('Node pair list unavailable: no bridge transport is connected. Configure bridge device settings and verify connectivity.');
                return [];
            }
            ...
            const runPairFlow = async () => {
                ...
                await Promise.all(connectedRuntimes.map(async (runtime) => {
                    const bridgeId = runtime.bridgeId;
                    const client = runtime.client;
                    if (!client)
                        return;
                    ...
                    let nodes = [];
                    try {
                        const nodeListResult = await this.withTimeout(client.getNodeList(), _a.PAIR_NODE_LIST_TIMEOUT_MS, `node list lookup (${bridgeId})`);
                    ...
                    if (app.resolveCompiledProfileEntry) {
                        try {
                            ...
                            }), _a.PAIR_ICON_INFERENCE_TIMEOUT_MS, `node icon inference (${bridgeId})`);
                        }
                        catch (error) {
                ...
                allCandidates.sort((left, right) => {
                    const leftBridge = left.data.bridgeId === pairing_1.ZWJS_DEFAULT_BRIDGE_ID ? '' : left.data.bridgeId;
                    const rightBridge = right.data.bridgeId === pairing_1.ZWJS_DEFAULT_BRIDGE_ID ? '' : right.data.bridgeId;
        ...
        async onRepair(session, device) {
            ...
            const loadSnapshot = async () => this.loadDeviceToolsSnapshot(app, homeyDeviceId);
        ...
        async loadDeviceToolsSnapshot(app, homeyDeviceId) {
            if (!homeyDeviceId) {
                throw new Error('Device Tools unavailable: node device ID is missing.');
            }
        ...
        parseDeviceToolsActionRequest(payload) {
            ...
            if (kind === 'extension') {
                ...
                if (!extensionId || !actionId) {
                    throw new Error('Invalid Device Tools extension action payload: extensionId and actionId are required.');
                }
            ...
            throw new Error('Invalid Device Tools action selection. Expected one of: auto, backfill-marker, adopt-recommended-baseline, none, or kind=extension payload.');
        }
    },
