...
exports.BASELINE_MARKER_PROJECTION_VERSION = exports.CURATION_SCHEMA_VERSION = exports.CURATION_SETTINGS_KEY = void 0;
exports.loadCurationRuntimeFromSettings = loadCurationRuntimeFromSettings;
exports.resolveCurationEntryFromRuntime = resolveCurationEntryFromRuntime;
exports.lowerCurationEntryToRuntimeActions = lowerCurationEntryToRuntimeActions;
exports.applyCurationEntryToProfile = applyCurationEntryToProfile;
exports.buildBaselineProfileProjectionV1 = buildBaselineProfileProjectionV1;
exports.computeBaselineProfileHashV1 = computeBaselineProfileHashV1;
exports.createBaselineMarkerV1 = createBaselineMarkerV1;
exports.evaluateBaselineRecommendationState = evaluateBaselineRecommendationState;
exports.upsertCurationBaselineMarkerV1 = upsertCurationBaselineMarkerV1;
exports.removeCurationEntryV1 = removeCurationEntryV1;
const crypto = require('node:crypto');
const CURATION_SETTINGS_KEY = 'curation.v1';
exports.CURATION_SETTINGS_KEY = CURATION_SETTINGS_KEY;
const CURATION_SCHEMA_VERSION = 'homey-curation/v1';
exports.CURATION_SCHEMA_VERSION = CURATION_SCHEMA_VERSION;
const BASELINE_MARKER_PROJECTION_VERSION = '1';
exports.BASELINE_MARKER_PROJECTION_VERSION = BASELINE_MARKER_PROJECTION_VERSION;
...
function ensureNoCollectionPairOverlap(collections, addKey, removeKey, pathLabel) {
    ...
    if (addItems.length === 0 || removeItems.length === 0)
        return;
    const addSet = new Set(addItems.map((item, index) => {
    ...
    for (let index = 0; index < removeItems.length; index += 1) {
        ...
        if (addSet.has(canonical)) {
            throw new Error(`${pathLabel} has overlapping values across ${addKey} and ${removeKey}; each value may appear in only one`);
        }
    }
}
function normalizeEntryOverrides(rawOverrides, pathLabel) {
    ...
    if (overridesRecord.deviceIdentity !== undefined) {
        const deviceIdentityRecord = assertPlainObject(overridesRecord.deviceIdentity, `${pathLabel}.deviceIdentity`);
        ...
        if (deviceIdentityRecord.homeyClass !== undefined) {
            normalizedIdentity.homeyClass = assertNonEmptyString(deviceIdentityRecord.homeyClass, `${pathLabel}.deviceIdentity.homeyClass`);
        }
        if (deviceIdentityRecord.driverTemplateId !== undefined) {
            normalizedIdentity.driverTemplateId = assertNonEmptyString(deviceIdentityRecord.driverTemplateId, `${pathLabel}.deviceIdentity.driverTemplateId`);
        }
        normalized.deviceIdentity = normalizedIdentity;
    }
    if (overridesRecord.capabilities !== undefined) {
        const capabilitiesRecord = assertPlainObject(overridesRecord.capabilities, `${pathLabel}.capabilities`);
        const normalizedCapabilities = {};
        for (const [capabilityId, capabilityOverride] of Object.entries(capabilitiesRecord)) {
            const normalizedCapabilityId = assertNonEmptyString(capabilityId, `${pathLabel}.capabilities key`);
            const capabilityRecord = assertPlainObject(capabilityOverride, `${pathLabel}.capabilities.${normalizedCapabilityId}`);
            assertAllowedKeys(capabilityRecord, CAPABILITY_OVERRIDE_KEYS, `${pathLabel}.capabilities.${normalizedCapabilityId}`);
            const normalizedCapabilityOverride = {};
            if (capabilityRecord.inboundMapping !== undefined) {
                normalizedCapabilityOverride.inboundMapping = assertPlainObject(capabilityRecord.inboundMapping, `${pathLabel}.capabilities.${normalizedCapabilityId}.inboundMapping`);
            }
            if (capabilityRecord.outboundMapping !== undefined) {
                normalizedCapabilityOverride.outboundMapping = assertPlainObject(capabilityRecord.outboundMapping, `${pathLabel}.capabilities.${normalizedCapabilityId}.outboundMapping`);
            }
            if (capabilityRecord.flags !== undefined) {
                normalizedCapabilityOverride.flags = assertPlainObject(capabilityRecord.flags, `${pathLabel}.capabilities.${normalizedCapabilityId}.flags`);
            }
    ...
    if (overridesRecord.collections !== undefined) {
        const collectionsRecord = assertPlainObject(overridesRecord.collections, `${pathLabel}.collections`);
        ...
        for (const key of COLLECTIONS_KEYS) {
            if (collectionsRecord[key] !== undefined) {
                normalizedCollections[key] = normalizeCollectionArray(collectionsRecord[key], `${pathLabel}.collections.${key}`);
            }
        }
        for (const [addKey, removeKey] of COLLECTION_PAIR_CONSTRAINTS) {
            ensureNoCollectionPairOverlap(normalizedCollections, addKey, removeKey, `${pathLabel}.collections`);
        }
...
function normalizeEntry(rawEntry, homeyDeviceId, pathLabel) {
    ...
    const targetHomeyDeviceId = assertNonEmptyString(targetDevice.homeyDeviceId, `${pathLabel}.targetDevice.homeyDeviceId`);
    if (targetHomeyDeviceId !== homeyDeviceId) {
        throw new Error(`${pathLabel}.targetDevice.homeyDeviceId must match entry key (${homeyDeviceId})`);
    }
    const baselineMarker = assertPlainObject(entryRecord.baselineMarker, `${pathLabel}.baselineMarker`);
    ...
    const normalized = {
        ...
        baselineMarker: {
            projectionVersion: assertNonEmptyString(baselineMarker.projectionVersion, `${pathLabel}.baselineMarker.projectionVersion`),
            baselineProfileHash: assertNonEmptyString(baselineMarker.baselineProfileHash, `${pathLabel}.baselineMarker.baselineProfileHash`),
            updatedAt: assertIsoTimestamp(baselineMarker.updatedAt, `${pathLabel}.baselineMarker.updatedAt`),
        },
        updatedAt: assertIsoTimestamp(entryRecord.updatedAt, `${pathLabel}.updatedAt`),
    };
    if (targetDevice.catalogId !== undefined) {
        normalized.targetDevice.catalogId = assertNonEmptyString(targetDevice.catalogId, `${pathLabel}.targetDevice.catalogId`);
    }
    if (targetDevice.diagnosticDeviceKey !== undefined) {
        normalized.targetDevice.diagnosticDeviceKey = assertNonEmptyString(targetDevice.diagnosticDeviceKey, `${pathLabel}.targetDevice.diagnosticDeviceKey`);
    }
    if (baselineMarker.pipelineFingerprint !== undefined) {
        normalized.baselineMarker.pipelineFingerprint = assertNonEmptyString(baselineMarker.pipelineFingerprint, `${pathLabel}.baselineMarker.pipelineFingerprint`);
    }
...
function normalizeCurationDocument(rawDocument) {
    ...
    const schemaVersion = assertNonEmptyString(topLevel.schemaVersion, 'curation document.schemaVersion');
    if (schemaVersion !== CURATION_SCHEMA_VERSION) {
        throw new Error(`curation document.schemaVersion must be ${CURATION_SCHEMA_VERSION} (received ${schemaVersion})`);
    }
    ...
    for (const [homeyDeviceId, rawEntry] of Object.entries(entriesRaw)) {
        const normalizedHomeyDeviceId = assertNonEmptyString(homeyDeviceId, 'curation document.entries key');
        entries[normalizedHomeyDeviceId] = normalizeEntry(rawEntry, normalizedHomeyDeviceId, `curation document.entries.${normalizedHomeyDeviceId}`);
    }
    return {
        schemaVersion,
        updatedAt,
...
function lowerCurationEntryToRuntimeActions(curationEntry, options = {}) {
    if (!isPlainObject(curationEntry) || !isPlainObject(curationEntry.overrides))
        return [];
    ...
    if (isPlainObject(overrides.deviceIdentity)) {
        if (typeof overrides.deviceIdentity.homeyClass === 'string') {
            actions.push(createCurationRuntimeAction(homeyDeviceId, 'set-device-identity', 'deviceIdentity.homeyClass', {
                slot: 'homeyClass',
                value: overrides.deviceIdentity.homeyClass,
            }));
        }
        if (typeof overrides.deviceIdentity.driverTemplateId === 'string') {
            actions.push(createCurationRuntimeAction(homeyDeviceId, 'set-device-identity', 'deviceIdentity.driverTemplateId', {
    ...
    if (Array.isArray(capabilitiesAdd)) {
        for (let index = 0; index < capabilitiesAdd.length; index += 1) {
            actions.push(createCurationRuntimeAction(homeyDeviceId, 'add-capability', `collections.capabilitiesAdd.${index}`, { capabilityId: capabilitiesAdd[index] }));
        }
    }
    if (isPlainObject(overrides.capabilities)) {
        ...
        for (const capabilityId of capabilityIds) {
            ...
            if (!isPlainObject(capabilityOverride))
                continue;
            if (capabilityOverride.inboundMapping !== undefined) {
                actions.push(createCurationRuntimeAction(homeyDeviceId, 'set-capability', `capabilities.${capabilityId}.inboundMapping`, {
                    capabilityId,
                    slot: 'inboundMapping',
            ...
            if (capabilityOverride.outboundMapping !== undefined) {
                actions.push(createCurationRuntimeAction(homeyDeviceId, 'set-capability', `capabilities.${capabilityId}.outboundMapping`, {
                    capabilityId,
            ...
            if (capabilityOverride.flags !== undefined) {
                actions.push(createCurationRuntimeAction(homeyDeviceId, 'set-capability', `capabilities.${capabilityId}.flags`, {
                    capabilityId,
                    slot: 'flags',
    ...
    if (Array.isArray(capabilitiesRemove)) {
        for (let index = 0; index < capabilitiesRemove.length; index += 1) {
            actions.push(createCurationRuntimeAction(homeyDeviceId, 'remove-capability', `collections.capabilitiesRemove.${index}`, { capabilityId: capabilitiesRemove[index] }));
        }
    }
    if (Array.isArray(subscriptionsAdd)) {
        for (let index = 0; index < subscriptionsAdd.length; index += 1) {
            actions.push(createCurationRuntimeAction(homeyDeviceId, 'add-subscription', `collections.subscriptionsAdd.${index}`, { value: subscriptionsAdd[index] }));
        }
    }
    if (Array.isArray(subscriptionsRemove)) {
        for (let index = 0; index < subscriptionsRemove.length; index += 1) {
            actions.push(createCurationRuntimeAction(homeyDeviceId, 'remove-subscription', `collections.subscriptionsRemove.${index}`, { value: subscriptionsRemove[index] }));
        }
    }
    if (Array.isArray(ignoredValuesAdd)) {
        for (let index = 0; index < ignoredValuesAdd.length; index += 1) {
            actions.push(createCurationRuntimeAction(homeyDeviceId, 'add-ignored-value', `collections.ignoredValuesAdd.${index}`, { value: ignoredValuesAdd[index] }));
        }
    }
    if (Array.isArray(ignoredValuesRemove)) {
        for (let index = 0; index < ignoredValuesRemove.length; index += 1) {
            actions.push(createCurationRuntimeAction(homeyDeviceId, 'remove-ignored-value', `collections.ignoredValuesRemove.${index}`, { value: ignoredValuesRemove[index] }));
        }
    }
    return actions;
}
...
function applyCollectionAdd(profile, slotKey, value, action, report) {
    if (!Array.isArray(profile[slotKey]))
        profile[slotKey] = [];
    ...
    for (const action of actions) {
        try {
            ...
            if (action.kind === 'add-capability') {
                const capabilityId = assertNonEmptyString(action.payload?.capabilityId, `${action.path}.capabilityId`);
...
function isValueIdLike(value) {
    if (!isPlainObject(value))
        return false;
    if (!Object.prototype.hasOwnProperty.call(value, 'commandClass'))
        return false;
...
function normalizeValueIdForBaselineHashV1(valueId) {
    const normalized = {
        ...
        endpoint: valueId.endpoint === undefined ? 0 : normalizeBaselineComparableNumber(valueId.endpoint),
...
function buildBaselineProfileProjectionV1(profile) {
    ...
    for (const rawCapability of rawCapabilities) {
        if (!isPlainObject(rawCapability) || typeof rawCapability.capabilityId !== 'string')
            continue;
...
function createBaselineMarkerV1(profile, options = {}) {
    ...
    if (pipelineFingerprint.length > 0)
        marker.pipelineFingerprint = pipelineFingerprint;
    return marker;
}
function normalizeStoredBaselineMarker(marker) {
    ...
    if (projectionVersion.length === 0)
        return null;
    if (baselineProfileHash.length === 0)
        return null;
...
function normalizeBaselineMarkerForStorage(marker, updatedAtFallback) {
    ...
    const normalizedMarker = {
        projectionVersion: assertNonEmptyString(markerRecord.projectionVersion, 'baselineMarker.projectionVersion'),
        baselineProfileHash: assertNonEmptyString(markerRecord.baselineProfileHash, 'baselineMarker.baselineProfileHash'),
        updatedAt: markerRecord.updatedAt !== undefined
            ? assertIsoTimestamp(markerRecord.updatedAt, 'baselineMarker.updatedAt')
            : updatedAtFallback,
    };
    if (markerRecord.pipelineFingerprint !== undefined) {
        normalizedMarker.pipelineFingerprint = assertNonEmptyString(markerRecord.pipelineFingerprint, 'baselineMarker.pipelineFingerprint');
    }
    return normalizedMarker;
}
function upsertCurationBaselineMarkerV1(document, homeyDeviceId, baselineMarker, options = {}) {
    ...
    if (!isPlainObject(nextEntry.overrides))
        nextEntry.overrides = {};
...
function removeCurationEntryV1(document, homeyDeviceId, options = {}) {
    ...
    const updatedAt = new Date(options.now ?? Date.now()).toISOString();
    const nextDocument = cloneCurationDocumentForMutation(document);
    const removed = Object.prototype.hasOwnProperty.call(nextDocument.entries, normalizedHomeyDeviceId);
    if (removed)
        delete nextDocument.entries[normalizedHomeyDeviceId];
    nextDocument.updatedAt = updatedAt;
    return {
        document: nextDocument,
        removed,
