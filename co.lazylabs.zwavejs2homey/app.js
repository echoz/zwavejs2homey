...
module.exports = (_a = class Zwavejs2HomeyApp extends homey_1.default.App {
        ...
        static toErrorMessage(error) {
            ...
            const normalized = _a.toStringOrNull(error);
        ...
        static normalizeIdentifierToken(value) {
            const normalized = _a.toStringOrNull(value);
            if (!normalized)
                return null;
            const token = normalized.toLowerCase().replace(/[^a-z0-9]/g, '');
            return token.length > 0 ? token : null;
        }
        static toValueIdSlotNumber(valueId) {
            const propertyKeySlot = _a.parseNumericIdentityOrNull(valueId.propertyKey);
            if (propertyKeySlot && propertyKeySlot > 0)
                return propertyKeySlot;
            const propertyKeyNameSlot = _a.parseNumericIdentityOrNull(valueId.propertyKeyName);
            if (propertyKeyNameSlot && propertyKeyNameSlot > 0)
                return propertyKeyNameSlot;
            return null;
        }
        static isCommandClass(valueId, commandClass) {
            return _a.parseNumericIdentityOrNull(valueId.commandClass) === commandClass;
        }
        static isLockUserIdStatusValueId(valueId) {
            if (!_a.isCommandClass(valueId, 99))
                return false;
            const propertyToken = _a.normalizeIdentifierToken(valueId.property);
            const propertyNameToken = _a.normalizeIdentifierToken(valueId.propertyName);
            return propertyToken === 'useridstatus' || propertyNameToken === 'useridstatus';
        }
        static isLockUserCodeValueId(valueId) {
            if (!_a.isCommandClass(valueId, 99))
                return false;
            const propertyToken = _a.normalizeIdentifierToken(valueId.property);
            const propertyNameToken = _a.normalizeIdentifierToken(valueId.propertyName);
        ...
        static isLockKeypadStateValueId(valueId) {
            if (!_a.isCommandClass(valueId, 113))
                return false;
            const propertyToken = _a.normalizeIdentifierToken(valueId.property);
            const propertyNameToken = _a.normalizeIdentifierToken(valueId.propertyName);
            const propertyKeyToken = _a.normalizeIdentifierToken(valueId.propertyKey);
            const propertyKeyNameToken = _a.normalizeIdentifierToken(valueId.propertyKeyName);
            const propertyHasAccessControl = propertyToken === 'accesscontrol' || propertyNameToken === 'accesscontrol';
            const keyHasKeypadState = propertyKeyToken === 'keypadstate' ||
        ...
        static toValueStatesMap(value) {
            const states = new Map();
            if (Array.isArray(value)) {
                value.forEach((entry, index) => {
                    const label = _a.toStringOrNull(entry);
                    if (!label)
                        return;
            ...
            for (const [key, rawLabel] of Object.entries(value)) {
                const label = _a.toStringOrNull(rawLabel);
                if (!label)
                    continue;
        ...
        static toStateLabel(value, valueId) {
            const asString = _a.toStringOrNull(value);
            if (asString)
                return asString;
            if (typeof value !== 'number' || !Number.isFinite(value))
                return null;
            const states = _a.toValueStatesMap(valueId.states);
            return states.get(String(value)) ?? null;
        }
        static classifyLockUserCodeSlotState(stateLabel) {
        ...
        static isLockoutStateLabel(stateLabel) {
            const token = stateLabel?.toLowerCase().replace(/[^a-z0-9]/g, '') ?? '';
            if (!token)
                return false;
            return (token.includes('lockout') ||
                token.includes('temporarydisabled') ||
                token.includes('keypaddisabled') ||
                token.includes('toomany'));
        }
        static toWritableStateValue(rawKey) {
            if (/^-?\d+$/.test(rawKey)) {
        ...
        static resolveLockUserCodeStatusValueForState(valueId, state) {
            const states = _a.toValueStatesMap(valueId.states);
            if (states.size === 0)
                return null;
            for (const [rawKey, label] of states.entries()) {
                if (_a.classifyLockUserCodeSlotState(label) !== state)
                    continue;
                return {
                    value: _a.toWritableStateValue(rawKey),
                    label,
                };
            }
            return null;
        ...
        getBridgeDiagnosticsRefreshStatus(bridgeId) {
        ...
        markBridgeDiagnosticsRefreshSuccess(bridgeId, reason) {
            const normalizedBridgeId = this.resolveBridgeId(bridgeId);
            this.bridgeDiagnosticsRefreshStatusById.set(normalizedBridgeId, {
                lastSuccessAt: new Date().toISOString(),
                lastFailureAt: null,
                lastFailureReason: null,
                lastReason: _a.toStringOrNull(reason),
            });
        }
        markBridgeDiagnosticsRefreshFailure(bridgeId, reason, error) {
            const normalizedBridgeId = this.resolveBridgeId(bridgeId);
            const status = this.getBridgeDiagnosticsRefreshStatus(normalizedBridgeId);
            this.bridgeDiagnosticsRefreshStatusById.set(normalizedBridgeId, {
                lastSuccessAt: status.lastSuccessAt,
                lastFailureAt: new Date().toISOString(),
                lastFailureReason: _a.toErrorMessage(error),
                lastReason: _a.toStringOrNull(reason),
            });
        }
        ...
        resolveBridgeIdFromNodeDeviceData(data) {
            return _a.toStringOrNull(data?.bridgeId) ?? this.resolveBridgeId(undefined);
        }
        ...
        async refreshBridgeRuntimeDiagnostics(reason) {
            try {
                ...
                let refreshError;
                for (const device of devices) {
                    const bridgeId = this.resolveBridgeIdFromBridgeDeviceData(device.getData?.()) ??
                    ...
                    this.markBridgeDiagnosticsRefreshFailure(bridgeId, reason, new Error('Bridge device missing onRuntimeDiagnosticsRefresh handler'));
                }
                if (refreshError) {
                    throw refreshError;
                }
        ...
        async deleteNodeDevicesForBridge(options) {
            ...
            const targetHomeyDeviceIds = new Set();
            for (const nodeDevice of nodeDevices) {
                ...
                if (nodeBridgeId !== bridgeId)
                    continue;
                const homeyDeviceId = _a.toStringOrNull(data?.id);
                if (!homeyDeviceId)
                    continue;
                targetHomeyDeviceIds.add(homeyDeviceId);
            }
            const deleteApi = this.homey.api && typeof this.homey.api.delete === 'function'
            ...
            for (const homeyDeviceId of targetHomeyDeviceIds) {
                try {
                    ...
                    const errorMessage = _a.toErrorMessage(error);
                    errors.push({
                        homeyDeviceId,
                        error: errorMessage,
                    });
        ...
        async readExtensionNodeValue(options) {
            const result = await options.client.getNodeValue(options.nodeId, options.valueId);
            if (result.success) {
                const value = (0, core_1.extractZwjsNodeValue)(result.result);
                return typeof value === 'undefined' ? null : value;
            }
            options.warnings.push(options.warningCode);
            return null;
        }
        async resolveLockUserCodeRuntime(options) {
            const warnings = [];
            ...
            const definedValueIds = (0, core_1.extractZwjsDefinedValueIds)(definedValueResult.result);
            const slotStatusValueIds = new Map();
            const slotCodeValueIds = new Map();
            const slotNumbers = new Set();
            let keypadStateValueId = null;
            for (const valueId of definedValueIds) {
                const slot = _a.toValueIdSlotNumber(valueId);
                if (slot && _a.isLockUserIdStatusValueId(valueId)) {
                    if (!slotStatusValueIds.has(slot))
                        slotStatusValueIds.set(slot, valueId);
                    slotNumbers.add(slot);
                    continue;
                }
                if (slot && _a.isLockUserCodeValueId(valueId)) {
                    if (!slotCodeValueIds.has(slot))
                        slotCodeValueIds.set(slot, valueId);
                    slotNumbers.add(slot);
                    continue;
                }
                if (!keypadStateValueId && _a.isLockKeypadStateValueId(valueId)) {
        ...
        async readLockUserCodeSlots(options) {
            ...
            for (const target of resolved.slotTargets) {
                ...
                const statusValue = statusValueId
                    ...
                    : null;
                const normalizedStatusValue = typeof statusValue === 'string' || typeof statusValue === 'number' ? statusValue : null;
                const stateLabel = statusValueId
                    ? _a.toStateLabel(statusValue, statusValueId)
                    : null;
                const state = statusValueId
                    ? _a.classifyLockUserCodeSlotState(stateLabel)
                    : 'unknown';
                slots.push({
                    slot: target.slot,
                    state,
            ...
            let keypadStateLabel = null;
            let lockoutActive = null;
            if (resolved.keypadStateValueId) {
                ...
                keypadStateLabel = _a.toStateLabel(keypadValue, resolved.keypadStateValueId);
                lockoutActive = _a.isLockoutStateLabel(keypadStateLabel);
            }
            const diagnostics = {
                ready: options.node.node.ready === true,
                valueIdsLoaded: true,
        ...
        registerProfileExtensionActionHandlers() {
            this.profileExtensionActionHandlers.clear();
            this.profileExtensionActionHandlers.set('lock-user-codes:set-user-code', async (options) => this.executeLockSetUserCodeAction(options));
            this.profileExtensionActionHandlers.set('lock-user-codes:remove-user-code', async (options) => this.executeLockRemoveUserCodeAction(options));
            this.profileExtensionActionHandlers.set('lock-user-codes:set-user-code-state', async (options) => this.executeLockSetUserCodeStateAction(options));
        }
        static toRequiredPositiveIntegerArg(args, name) {
        ...
        static toRequiredNonEmptyStringArg(args, name) {
            const value = _a.toStringOrNull(args[name]);
            return value && value.length > 0 ? value : null;
        }
        static toLockUserCodeStateArg(args) {
            const value = _a.toStringOrNull(args.state);
            if (!value)
                return null;
            const normalized = value.trim().toLowerCase();
        ...
        async executeProfileExtensionWrites(options) {
        ...
        async executeLockSetUserCodeAction(options) {
            const slot = _a.toRequiredPositiveIntegerArg(options.args, 'slot');
            if (!slot) {
                return {
                    executed: false,
                    reason: 'invalid-slot',
            ...
            const code = _a.toRequiredNonEmptyStringArg(options.args, 'code');
            if (!code) {
                return {
                    executed: false,
                    reason: 'invalid-code',
        ...
        async executeLockRemoveUserCodeAction(options) {
            const slot = _a.toRequiredPositiveIntegerArg(options.args, 'slot');
            if (!slot) {
                return {
                    executed: false,
            ...
            if (target.statusValueId) {
                const availableState = _a.resolveLockUserCodeStatusValueForState(target.statusValueId, 'available');
                if (availableState) {
                    return this.executeProfileExtensionWrites({
                        client: resolved.client,
                        nodeId: resolved.nodeId,
        ...
        async executeLockSetUserCodeStateAction(options) {
            const slot = _a.toRequiredPositiveIntegerArg(options.args, 'slot');
            if (!slot) {
                return {
                    executed: false,
            ...
            const state = _a.toLockUserCodeStateArg(options.args);
            if (!state) {
                return {
                    executed: false,
            ...
            const mappedValue = _a.resolveLockUserCodeStatusValueForState(target.statusValueId, state);
            if (!mappedValue) {
                return {
                    executed: false,
