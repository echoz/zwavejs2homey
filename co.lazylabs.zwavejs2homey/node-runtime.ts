...
function coerceAccessControlNonzeroStateTransform(value: unknown): boolean | undefined {
  const numeric = normalizeNumericValue(value);
  if (numeric === undefined) return undefined;
  return numeric !== 0;
}

function coerceAccessControlLockStateProblemTransform(value: unknown): boolean | undefined {
  const numeric = normalizeNumericValue(value);
  if (numeric === undefined) return undefined;
  if (numeric === 0) return false;
  if (numeric === 11) return true;
  return numeric > 0;
}
