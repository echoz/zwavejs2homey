...
export class FallbackNormalizer implements ZwjsProtocolAdapter {
  ...
  normalizeIncoming(message: unknown) {
    ...
    const responseId = typeof record.messageId === 'string' ? record.messageId : typeof record.id === 'string' ? record.id : undefined;
    const version =
    ...
    return {
      ...
      requestResponse:
        responseId && !isFailedResult ? { id: responseId, payload: 'result' in record ? record.result : message } : undefined,
      requestError: responseId && isFailedResult ? { id: responseId, error: 'error' in record ? record.error : record } : undefined,
      events: [
        {
