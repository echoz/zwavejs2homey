...
export interface HomeyInboundMapping {
  ...
  selector: NormalizedZwaveValueId | { eventType: string };
  transformRef?: string;
  transformParams?: Record<string, unknown>;
  watchers?: Array<NormalizedZwaveValueId | { eventType: string }>;
}
...
export interface HomeyOutboundMapping {
  ...
  target: NormalizedZwaveValueId | { command: string; argsTemplate?: Record<string, unknown> };
  transformRef?: string;
  transformParams?: Record<string, unknown>;
  validation?: {
    min?: number;
...
export interface CompiledHomeyProfilePlan {
  ...
  subscriptions?: Array<{ eventType: string } | NormalizedZwaveValueId>;
