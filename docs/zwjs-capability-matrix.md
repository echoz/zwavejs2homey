...
| node | `node.get_value_metadata` | 0+ | yes | backend metadata handling | `typed-wrapper` (`getNodeValueMetadata`) | `partial` | `fixture`, `mocked-transport` | no | P0 | Common metadata fields typed; nested payload remains protocol-native |
| node | `node.get_value` | 14+ | yes | backend value reads/writes | `typed-wrapper` (`getNodeValue`) | `protocol-native` | `fixture`, `mocked-transport` | no | P0 | Value type intentionally generic |
| node | `node.get_value_timestamp` | 27+ | yes | backend value tracking | `typed-wrapper` (`getNodeValueTimestamp`) | `partial` | `fixture`, `mocked-transport` | no | P1 | Returns scalar, but semantics vary |
...
| controller | `nvm backup progress` | yes | backend emits progress | `zwjs.event.controller` + raw-normalized | none | none | no | P2 | Add specialized payload type if backup workflows prioritized |
...
- Additional specialized node event typing needed for capability sync (beyond current `value updated` / `metadata updated` / `notification` as real traffic reveals)
- Live validation for `getNodeValue`, `getNodeValueMetadata`, `getNodeValueTimestamp` on representative nodes/value IDs
