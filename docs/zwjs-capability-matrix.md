...
| zniffer.\*          | documented command family + events                                           | backend has `ZnifferManager` and Socket.IO zniffer events    | typed read subset implemented (`captured_frames`, `get_capture_as_zlf_buffer`, `supported_frequencies`, `current_frequency`); generic mutation path remains for lifecycle/frequency commands; specialized zniffer event typing added for common events | P3       | Lifecycle/frequency mutation wrappers still pending         |
...
| zniffer    | `ready` / `corrupted frame` / `frame` / `error` | yes                                       | UI has `ZNIFFER_FRAME`, `ZNIFFER_STATE`                         | `zwjs.event.zniffer` + raw-normalized    | `zwjs.event.zniffer.ready`, `.corrupted-frame`, `.frame`, `.error` | `fixture`, `normalizer` | no                                      | P3       | Core zniffer event subset typed; lifecycle wrappers and live validation pending            |
...
| `ZNIFFER_FRAME` / `ZNIFFER_STATE`                               | UI zniffer streaming/state      | zniffer domain plus app state           | typed zniffer event subset (`ready`, `corrupted frame`, `frame`, `error`) + generic `zwjs.event.zniffer` | Lifecycle/frequency wrappers still pending                                       |
