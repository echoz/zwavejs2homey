...
| driver   | `driver.update_log_config`                                          |         4+ | yes              | `driverFunction` / config methods in backend            | `typed-wrapper` (`updateDriverLogConfig`)               | `partial`         | `fixture`, `mocked-transport` (policy path)     | no              | P2       | Mutation-gated wrapper implemented; live validation pending                                                       |
| driver   | `driver.check_for_config_updates`                                   |         5+ | yes              | `checkForConfigUpdates`                                 | `typed-wrapper` (`checkDriverConfigUpdates`)            | `partial`         | `fixture`, `mocked-transport`                   | no              | P2       | High operational value; live validation pending                                                                   |
| driver   | `driver.install_config_update`                                      |         5+ | yes              | `installConfigUpdate`                                   | `typed-wrapper` (`installDriverConfigUpdate`)           | `partial`         | `fixture`, `mocked-transport` (policy path)     | no              | P2       | Mutation-gated wrapper implemented; operational safeguards still apply                                            |
...
| controller | `controller.get_known_lifeline_routes`                              |                                 varies | source command enum/docs family                   | route/health maintenance workflows                                  | `typed-wrapper` (`getControllerKnownLifelineRoutes`)                           | `partial`         | `fixture`, `mocked-transport`, `live-readonly`                   | yes             | P3       | Live-validated 2026-02-23; observed empty `routes` object on production instance |
| controller | `controller.get_rf_region`                                          |                                 varies | source command enum/docs family                   | RF region management workflows                                      | `typed-wrapper` (`getControllerRfRegion`)                                      | `partial`         | `fixture`, `mocked-transport`, `live-readonly`                   | yes             | P3       | Live-validated 2026-02-23; observed numeric region code (`9`)                    |
| controller | `controller.get_powerlevel`                                         |                                 varies | source command enum/docs family                   | powerlevel diagnostics workflows                                    | `typed-wrapper` (`getControllerPowerlevel`)                                    | `partial`         | `fixture`, `mocked-transport`, `live-readonly`                   | yes             | P3       | Live-validated 2026-02-23 (`powerlevel: 0`, `measured0dBm: 0`)                   |
| controller | `controller.get_max_long_range_powerlevel`                          |                                 varies | source command enum/docs family                   | long-range diagnostics workflows                                    | `typed-wrapper` (`getControllerMaxLongRangePowerlevel`)                        | `partial`         | `fixture`, `mocked-transport`, `live-readonly`                   | yes             | P3       | Live-validated 2026-02-23; observed `limit: 0`                                   |
| controller | `controller.get_long_range_channel`                                 |                                 varies | source command enum/docs family                   | long-range diagnostics workflows                                    | `typed-wrapper` (`getControllerLongRangeChannel`)                              | `partial`         | `fixture`, `mocked-transport`, `live-readonly`                   | yes             | P3       | Live-validated 2026-02-23; observed `channel: 255`, auto-channel supported       |
...
| node     | `node.get_value_metadata`                                                            |           0+ | yes              | backend metadata handling                                          | `typed-wrapper` (`getNodeValueMetadata`)                                                     | `partial`         | `fixture`, `mocked-transport`, `live-readonly` | yes                    | P0       | Extended metadata typing + runtime guards now cover observed fields (`minLength`, `valueSize`, `format`, `ccSpecific`, config flags) |
| node     | `node.get_value`                                                                     |          14+ | yes              | backend value reads/writes                                         | `typed-wrapper` (`getNodeValue`)                                                             | `partial`         | `fixture`, `mocked-transport`, `live-readonly` | yes                    | P0       | Live-validated envelope helpers plus observed command-class sample guards (`duration`, lock-handle flags, firmware versions)         |
...
| ---------- | ------------------------------------------------------- | ----------------------------------------------- | --------------------------------------------------------------- | ---------------------------------------- | ----------------------------------------------------------------------------- | ----------------------- | --------------------------------------- | -------- | -------------------------------------------------------------------------------------- |
| driver     | `logging`                                               | yes                                             | UI has `DEBUG`/log-related Socket.IO outputs                    | `zwjs.event.driver` + raw-normalized     | `zwjs.event.driver.logging`                                                   | `fixture`, `normalizer` | yes (observed on 2026-03-01)            | P1       | Live capture confirmed active payload shape and typed-field coverage baseline          |
...
| driver     | `log config updated`                                    | yes                                             | backend updates UI state via socket events                      | `zwjs.event.driver` + raw-normalized     | `zwjs.event.driver.log-config-updated`                                        | `fixture`, `normalizer` | not recorded                            | P2       | Specialized typing added; pair with `updateDriverLogConfig` live validation later      |
...
| controller | `grant security classes`                                | yes                                             | UI has `GRANT_SECURITY_CLASSES`                                 | `zwjs.event.controller` + raw-normalized | `zwjs.event.controller.grant-security-classes`                                | `fixture`, `normalizer` | no                                      | P2       | Specialized typing added; protocol inclusion/exclusion wrappers implemented            |
| controller | `validate dsk and enter pin`                            | yes                                             | UI has `VALIDATE_DSK`                                           | `zwjs.event.controller` + raw-normalized | `zwjs.event.controller.validate-dsk-and-enter-pin`                            | `fixture`, `normalizer` | no                                      | P2       | Specialized typing added; protocol inclusion/exclusion wrappers implemented            |
| controller | `inclusion aborted`                                     | yes                                             | UI has `INCLUSION_ABORTED`                                      | `zwjs.event.controller` + raw-normalized | `zwjs.event.controller.inclusion-aborted`                                     | `fixture`, `normalizer` | no                                      | P2       | Specialized typing added; protocol inclusion/exclusion wrappers implemented            |
...
- none (all current code-actionable gaps are closed as of March 1, 2026)

### Recently Closed

- `ZWJS-G1` (P1) closed on March 1, 2026:
  - captured active `driver.logging` traffic from live instance
  - exported JSON summary + NDJSON payload artifacts via `zwjs:inspect logs capture`
  - validated baseline observed payload keys and tightened specialized guard coverage
- `ZWJS-G4` (P0) closed on March 1, 2026:
  - sampled live node value/value-metadata payloads across representative nodes
  - expanded typed metadata surface + runtime metadata guards for observed fields
  - added command-class sample guards for observed value payload families:
    - CC 37/38 `duration` object values
    - CC 98 lock handle flags boolean arrays
    - CC 134 firmware versions string arrays
  - added observed fixtures and regression tests for these payload classes

`ZWJS-G1` capture command:
...
- if `driver.logging events` is `0`, generate traffic that emits active driver logs and rerun capture

### On-Demand Expansion Gaps

- `ZWJS-G1-FOLLOWUP`: capture additional `driver.logging` payload variants as they appear and extend fixtures only when stable new fields are observed
- `ZWJS-G4-FOLLOWUP`: capture additional command-class-specific value/value-metadata variants and expand typed sample guards only when stable patterns recur
